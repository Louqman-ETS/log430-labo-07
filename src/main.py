...
class CaisseMagasinApp:
    ...
    def ajouter_produit_vente(self):
        ...
        try:
            ...
            succes, message = VenteService.ajouter_produit(
                self.vente_en_cours, produit_id, quantite
            )
            print(message)
    ...
    def effectuer_retour_produits(self):
        ...
        if self.vente_en_cours is not None:
            ...
            choix = input(
                "Voulez-vous annuler cette vente et retourner les produits? (o/n): "
            )
            if choix.lower() == "o":
        ...
        try:
            ...
            confirmation = input(
                f"Confirmer le retour de la vente #{vente_id}? (o/n): "
            )
    ...
    def afficher_produits(self, produits: List[Dict[str, Any]]):
        ...
        print(
            f"{'ID':<4} | {'Code':<8} | {'Nom':<20} | {'Prix':<8} | {'Stock':<6} | {'Catégorie':<15}"
        )
        print("-" * 70)

        for p in produits:
            print(
                f"{p['id']:<4} | {p['code']:<8} | {p['nom']:<20} | {p['prix']:<8.2f} | {p['stock']:<6} | {p['categorie']:<15}"
            )
    ...
    def afficher_recu(self, vente: Dict[str, Any]):
        ...
        for ligne in vente["lignes"]:
            print(
                f"{ligne['produit']:<25} | {ligne['quantite']:<4} | {ligne['prix_unitaire']:<8.2f} | {ligne['sous_total']:<8.2f}"
            )
    ...
    def executer(self):
        ...
        while True:
            ...
            try:
                choix_num = int(choix)
                if (
                    1 <= choix_num <= len(options) - 1
                ):  # -1 car "Quitter" est toujours à la fin
