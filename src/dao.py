...
class ProduitDAO:
    ...
    def update_stock(db: Session, produit_id: int, quantity: int) -> bool:
        produit = (
            db.query(Produit).filter(Produit.id == produit_id).with_for_update().first()
        )
        if not produit:
            return False
...
class VenteDAO:
    ...
    @staticmethod
    def add_product_to_vente(
        db: Session, vente_id: int, produit_id: int, quantite: int
    ) -> Optional[LigneVente]:
        produit = (
            db.query(Produit).filter(Produit.id == produit_id).with_for_update().first()
        )
    ...
    @staticmethod
    def get_vente_with_lignes(
        db: Session, vente_id: int
    ) -> Optional[Tuple[Vente, List[LigneVente]]]:
