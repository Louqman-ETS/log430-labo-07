...
ACTIVE_REQUESTS = Gauge(
    "api_active_requests", "Number of active requests", ["instance_id"]
)

# Métriques temps réel (non cumulatives)
CURRENT_RPS = Gauge(
    "api_current_rps", "Current requests per second (calculated)", ["instance_id"]
)

ERROR_COUNT = Counter(
    "api_errors_total", "Total API errors", ["error_type", "endpoint", "instance_id"]
)

HEALTH_STATUS = Gauge(
    "api_health_status", "API health status (1=healthy, 0=unhealthy)", ["instance_id"]
)

CPU_USAGE = Gauge(
    "api_cpu_usage_percent", "CPU usage percentage of API process", ["instance_id"]
)

MEMORY_USAGE = Gauge(
    "api_memory_usage_bytes", "Memory usage in bytes of API process", ["instance_id"]
)
...
CACHE_HIT_RATIO = Gauge(
    "api_cache_hit_ratio", "Cache hit ratio (hits / (hits + misses))", ["instance_id"]
)

CACHE_KEYS_COUNT = Gauge(
    "api_cache_keys_total", "Total number of keys in cache", ["instance_id"]
)

CACHE_MEMORY_USAGE = Gauge(
    "api_cache_memory_bytes", "Memory usage of Redis cache in bytes", ["instance_id"]
)
...
class MetricsService:
        ...
        def monitor():
            while True:
                try:
                    ...
                    MEMORY_USAGE.labels(instance_id=INSTANCE_ID).set(
                        memory_info.rss
                    )  # Resident Set Size en bytes
    ...
    def _update_current_rps(self):
        ...
        current_count = REQUEST_COUNT.labels(
            method="GET", endpoint="/", status_code=200, instance_id=INSTANCE_ID
        )._value._value

        if (
            hasattr(self, "last_rps_update") and (now - self.last_rps_update) >= 1.0
        ):  # Mise à jour chaque seconde
            rps = (current_count - self.last_request_count) / (
...
def record_cache_operation(operation: str, result: str):
    ...
    CACHE_OPERATIONS.labels(
        operation=operation, instance_id=INSTANCE_ID, result=result
    ).inc()
