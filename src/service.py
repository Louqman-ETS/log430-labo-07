...
class VenteService:
    ...
    @staticmethod
    def ajouter_produit(vente_id: int, produit_id: int, quantite: int = 1) -> Tuple[bool, str]:
        with get_db_session() as db:
            try:
                produit = ProduitDAO.get_by_id(db, produit_id)
                if not produit:
                ...
                if produit.quantite_stock < quantite:
                    return False, f"Stock insuffisant: {produit.quantite_stock} disponible(s)"
                
                ligne = VenteDAO.add_product_to_vente(db, vente_id, produit_id, quantite)
                if not ligne:
    ...
    def effectuer_retour(vente_id: int) -> Tuple[bool, str]:
        with get_db_session() as db:
            try:
                ...
                if success:
                    return True, f"Retour effectué pour la vente #{vente_id} (montant: {montant_total:.2f} €). Produits remis en stock."
                else:
