...
INSTANCE_ID = os.getenv("INSTANCE_ID", "reporting-api-1")
...
REQUEST_DURATION = Histogram(
    "reporting_api_request_duration_seconds",
...
ACTIVE_REQUESTS = Gauge(
    "reporting_api_active_requests", "Number of active requests", ["instance_id"]
)

# Métriques temps réel (non cumulatives)
CURRENT_RPS = Gauge(
    "reporting_api_current_rps", "Current requests per second", ["instance_id"]
)
...
CPU_USAGE = Gauge(
    "reporting_api_cpu_usage_percent", "CPU usage percentage", ["instance_id"]
)

MEMORY_USAGE = Gauge(
    "reporting_api_memory_usage_bytes", "Memory usage in bytes", ["instance_id"]
)

MEMORY_USAGE_PERCENT = Gauge(
    "reporting_api_memory_usage_percent", "Memory usage percentage", ["instance_id"]
)
...
REPORT_GENERATION_DURATION = Histogram(
    "reporting_api_report_generation_duration_seconds",
...
EXTERNAL_API_DURATION = Histogram(
    "reporting_api_external_api_duration_seconds",
...
class MetricsService:
    ...
    def record_external_api_call(
        ...
        EXTERNAL_API_DURATION.labels(
            service=service, endpoint=endpoint, instance_id=INSTANCE_ID
        ).observe(duration)
