...
app = FastAPI(
    title="Stores API",
    description="API RESTful de gestion des magasins et caisses - Architecture DDD",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def startup_event():
    ...
    logger.info("🚀 Starting Stores API with enhanced logging and error handling")
    max_retries = 30
    retry_interval = 2
    ...
    for attempt in range(max_retries):
        try:
            ...
            init_database()
            logger.info("✅ Stores database initialized with sample data")
            break
...
async def shutdown_event():
    logger.info("🛑 Shutting down Stores API")
...
async def root():
    ...
    return {
        ...
        "version": "1.0.0",
        "docs": "/docs",
        "features": ["RESTful API", "DDD Architecture", "Store Management", "Structured Logging"]
    }
